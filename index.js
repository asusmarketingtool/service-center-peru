...
/**
 * Dado un string de dirección, devuelve la URL para embeber un mapa de Google Maps
 * sin requerir clave de API. El front-end podrá usar esa URL en un <iframe>.
 */
...
app.post('/nearest', (req, res) => {
  ...
  if (matches.length > 0) {
    const centersWithMap = matches.map(sc => ({
      ...sc,
      mapUrl: generateMapEmbedUrl(sc.address)
    }));
